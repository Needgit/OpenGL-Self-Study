...
void GLmodel::loadModel(std::string path)
{
    ...
    if(!scene || (scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE) || !scene->mRootNode) // parantesis added
    {
...
GLmesh GLmodel::processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	std::vector<GLmesh::Texture> textures;
    ...
    if(mesh->mMaterialIndex >= 0)
    {
        ...
        std::vector<GLmesh::Texture> diffuseMaps =
        		loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse");
        textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());

        std::vector<GLmesh::Texture> specularMaps =
...
std::vector<GLmesh::Texture> GLmodel::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
{
	std::vector<GLmesh::Texture> textures;
